...
param_grid = [{'kernel': ['linear'], 'C': [1, 10, 100, 1000]},
              {'kernel': ['rbf'], 'gamma': [
                  1e-3, 1e-4, 'scale'], 'C': [1, 10, 100, 1000]},
              {'kernel': ['poly'],  'gamma': [
                  1e-3, 1e-4, 'scale'], 'C': [1, 10, 100, 1000]},
...
def cross_validate(df):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df.iloc[:, 1:10], df["Loan_Status"], test_size=0.2, random_state=1)
    print("# Tuning hyper-parameters")
    print()
    loan_approved = y_train.filter(regex='1', axis=0).count()
    print(loan_approved)
    loan_disapproved = y_train.filter(regex='0', axis=0).count()
    print(loan_disapproved)
    print(y_train)
